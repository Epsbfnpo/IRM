...
class ERMPlusPlus(Algorithm,ErmPlusPlusMovingAvg):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        if self.hparams["lars"]:
            self.optimizer = LARS(self.network.parameters(), lr=self.hparams["lr"], weight_decay=self.hparams['weight_decay'], foreach=False)
        else:
            self.optimizer = torch.optim.Adam(self.network.parameters(), lr=self.hparams["lr"], weight_decay=self.hparams['weight_decay'], foreach=False)
        linear_parameters = []
        for n, p in self.network[1].named_parameters():
            linear_parameters.append(p)
        if self.hparams["lars"]:
            self.linear_optimizer = LARS(linear_parameters, lr=self.hparams["linear_lr"], weight_decay=self.hparams['weight_decay'], foreach=False)
        else:
            self.linear_optimizer = torch.optim.Adam(linear_parameters, lr=self.hparams["linear_lr"], weight_decay=self.hparams['weight_decay'], foreach=False)
        self.lr_schedule = []
...
class URM(ERM):
    ...
    def _setup_adversarial_net(self):
        ...
        if self.hparams["urm_discriminator_optimizer"] == 'sgd':
            self.discriminator_opt = torch.optim.SGD(self.discriminator.parameters(), lr=self.hparams['urm_discriminator_lr'], weight_decay=self.hparams['weight_decay'], momentum=0.9)
        elif self.hparams["urm_discriminator_optimizer"] == 'adam':
            self.discriminator_opt = torch.optim.Adam(self.discriminator.parameters(), lr=self.hparams['urm_discriminator_lr'], weight_decay=self.hparams['weight_decay'])
        else:
...
class AbstractDANN(Algorithm):
    def __init__(self, input_shape, num_classes, num_domains, hparams, conditional, class_balance):
        ...
        self.disc_opt = torch.optim.Adam((list(self.discriminator.parameters()) + list(self.class_embeddings.parameters())), lr=self.hparams["lr_d"], weight_decay=self.hparams['weight_decay_d'], betas=(self.hparams['beta1'], 0.9))
        self.gen_opt = torch.optim.Adam((list(self.featurizer.parameters()) + list(self.classifier.parameters())), lr=self.hparams["lr_g"], weight_decay=self.hparams['weight_decay_g'], betas=(self.hparams['beta1'], 0.9))
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        disc_labels = torch.cat([torch.full((x.shape[0], ), i, dtype=torch.int64, device=device) for i, (x, y) in enumerate(minibatches)])
        ...
        input_grad = autograd.grad(F.cross_entropy(disc_out, disc_labels, reduction='sum'), [disc_input], create_graph=True)[0]
...
class CDANN(AbstractDANN):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(CDANN, self).__init__(input_shape, num_classes, num_domains, hparams, conditional=True, class_balance=True)
...
class IRM(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        device = "cuda" if minibatches[0][0].is_cuda else "cpu"
        penalty_weight = (self.hparams['irm_lambda'] if self.update_count >= self.hparams['irm_penalty_anneal_iters'] else 1.0)
        nll = 0.
        penalty = 0.
        all_x = torch.cat([x for x, y in minibatches])
...
class RDM(ERM):
    ...
    def update(self, minibatches, unlabeled=None, held_out_minibatches=None):
        matching_penalty_weight = (self.hparams['rdm_lambda'] if self.update_count >= self.hparams['rdm_penalty_anneal_iters'] else 0.)
        variance_penalty_weight = (self.hparams['variance_weight'] if self.update_count >= self.hparams['rdm_penalty_anneal_iters'] else 0.)
        all_x = torch.cat([x for x, y in minibatches])
        ...
        if self.update_count == self.hparams['rdm_penalty_anneal_iters']:
            self.optimizer = torch.optim.Adam(self.network.parameters(), lr=self.hparams["rdm_lr"], weight_decay=self.hparams['weight_decay'])
        self.optimizer.zero_grad()
        total_loss.backward()
        self.optimizer.step()
        self.update_count += 1
...
class MLDG(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        for (xi, yi), (xj, yj) in split_meta_train_test(minibatches, self.num_meta_test):
            ...
            inner_opt = torch.optim.Adam(inner_net.parameters(), lr=self.hparams["lr"], weight_decay=self.hparams['weight_decay'])
...
class MTL(Algorithm):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = networks.Classifier(self.featurizer.n_outputs * 2, num_classes, self.hparams['nonlinear_classifier'])
        self.optimizer = torch.optim.Adam(list(self.featurizer.parameters()) + list(self.classifier.parameters()), lr=self.hparams["lr"], weight_decay=self.hparams['weight_decay'])
...
class SagNet(Algorithm):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.network_c = networks.Classifier(self.network_f.n_outputs, num_classes, self.hparams['nonlinear_classifier'])
        self.network_s = networks.Classifier(self.network_f.n_outputs, num_classes, self.hparams['nonlinear_classifier'])
...
class SelfReg(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.cdpl = nn.Sequential(nn.Linear(input_feat_size, hidden_size), nn.BatchNorm1d(hidden_size), nn.ReLU(inplace=True), nn.Linear(hidden_size, hidden_size), nn.BatchNorm1d(hidden_size), nn.ReLU(inplace=True), nn.Linear(hidden_size, input_feat_size), nn.BatchNorm1d(input_feat_size))
...
class SANDMask(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.k = hparams["k"]
        betas = (0.9, 0.999)
        self.optimizer = torch.optim.Adam(self.network.parameters(), lr=self.hparams["lr"], weight_decay=self.hparams['weight_decay'], betas=betas)
        self.register_buffer('update_count', torch.tensor([0]))
...
class Fishr(Algorithm):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = extend(networks.Classifier(self.featurizer.n_outputs, num_classes, self.hparams['nonlinear_classifier'],))
        self.network = nn.Sequential(self.featurizer, self.classifier)
        self.register_buffer("update_count", torch.tensor([0]))
        self.bce_extended = extend(nn.CrossEntropyLoss(reduction='none'))
        self.ema_per_domain = [MovingAverage(ema=self.hparams["ema"], oneminusema_correction=True) for _ in range(self.num_domains)]
    ...
    def _compute_distance_grads_var(self, grads_var_per_domain):
        grads_var = OrderedDict([(name, torch.stack([grads_var_per_domain[domain_id][name] for domain_id in range(self.num_domains)], dim=0).mean(dim=0)) for name in grads_var_per_domain[0].keys()])
        penalty = 0
        for domain_id in range(self.num_domains):
...
class TRM(Algorithm):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.olist = [torch.optim.SGD(self.clist[i].parameters(), lr=1e-1,) for i in range(num_domains+1)]
        self.optimizer_f = torch.optim.Adam(self.featurizer.parameters(), lr=self.hparams["lr"], weight_decay=self.hparams['weight_decay'])
        self.optimizer_c = torch.optim.Adam(self.classifier.parameters(), lr=self.hparams["lr"], weight_decay=self.hparams['weight_decay'])
...
class IB_ERM(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        device = "cuda" if minibatches[0][0].is_cuda else "cpu"
        ib_penalty_weight = (self.hparams['ib_lambda'] if self.update_count >= self.hparams['ib_penalty_anneal_iters'] else 0.0)
        nll = 0.
        ib_penalty = 0.
        all_x = torch.cat([x for x, y in minibatches])
...
class IB_IRM(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        device = "cuda" if minibatches[0][0].is_cuda else "cpu"
        irm_penalty_weight = (self.hparams['irm_lambda'] if self.update_count >= self.hparams['irm_penalty_anneal_iters'] else 1.0)
        ib_penalty_weight = (self.hparams['ib_lambda'] if self.update_count >= self.hparams['ib_penalty_anneal_iters'] else 0.0)
        nll = 0.
        irm_penalty = 0.
        ib_penalty = 0.
        ...
        self.optimizer.step()
        self.update_count += 1
        return {'loss': loss.item(), 'nll': nll.item(), 'IRM_penalty': irm_penalty.item(), 'IB_penalty': ib_penalty.item()}
...
class AbstractCAD(Algorithm):
    def __init__(self, input_shape, num_classes, num_domains, hparams, is_conditional):
        ...
        if self.is_project:
            self.project = nn.Sequential(nn.Linear(feature_dim, feature_dim), nn.ReLU(inplace=True), nn.Linear(feature_dim, 128),)
    ...
    def bn_loss(self, z, y, dom_labels):
        ...
        if not self.is_conditional:
            ...
            if self.is_flipped:
                bn_loss = - (self.temperature / self.base_temperature) * torch.logsumexp(log_prob + (~mask_d).float().log(), dim=1)
            else:
                bn_loss = (self.temperature / self.base_temperature) * torch.logsumexp(log_prob + (mask_d).float().log(), dim=1)
        else:
            if self.is_flipped:
            ...
            if self.is_flipped:
                bn_loss = - (self.temperature / self.base_temperature) * torch.logsumexp(log_prob_y + mask_y_n_d.log(), dim=1)
            else:
                bn_loss = (self.temperature / self.base_temperature) * torch.logsumexp(log_prob_y + mask_y_d.log(), dim=1)
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        all_y = torch.cat([y for x, y in minibatches])
        all_z = self.featurizer(all_x)
        all_d = torch.cat([torch.full((x.shape[0],), i, dtype=torch.int64, device=device) for i, (x, y) in enumerate(minibatches)])
...
class Transfer(Algorithm):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = networks.Classifier(self.featurizer.n_outputs, num_classes, self.hparams['nonlinear_classifier'])
        self.adv_classifier = networks.Classifier(self.featurizer.n_outputs, num_classes, self.hparams['nonlinear_classifier'])
...
class EQRM(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        if self.update_count == self.hparams['eqrm_burnin_iters']:
            self.optimizer = torch.optim.Adam(self.network.parameters(), lr=self.hparams["eqrm_lr"], weight_decay=self.hparams['weight_decay'])
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        self.update_count += 1
...
class ADRMX(Algorithm):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.disc_opt = torch.optim.Adam((list(self.discriminator.parameters())), lr=self.hparams["lr"], betas=(self.hparams['beta1'], 0.9))
        self.opt = torch.optim.Adam((list(self.featurizer_label.parameters()) + list(self.featurizer_domain.parameters()) + list(self.classifier_label_1.parameters()) + list(self.classifier_label_2.parameters()) + list(self.classifier_domain.parameters())), lr=self.hparams["lr"], betas=(self.hparams['beta1'], 0.9))
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        disc_labels = torch.cat([torch.full((x.shape[0], ), i, dtype=torch.int64, device=all_x.device) for i, (x, _) in enumerate(minibatches)])
        ...
        if self.update_count.item() % (1 + d_steps_per_g) < d_steps_per_g:
            ...
            gen_loss = (classifier_loss_int + classifier_loss_final + self.hparams["dclf_lambda"] * classifier_loss_domain + self.hparams["rmxd_lambda"] * classifier_remixed_loss + self.hparams['cnt_lambda'] * (cnt_loss_int + cnt_loss_final) + (self.hparams['disc_lambda'] * -disc_loss))
            self.disc_opt.zero_grad()
            self.opt.zero_grad()
            gen_loss.backward()
            self.opt.step()
            return {'loss_total': gen_loss.item(), 'loss_cnt_int': cnt_loss_int.item(), 'loss_cnt_final': cnt_loss_final.item(), 'loss_clf_int': classifier_loss_int.item(), 'loss_clf_fin': classifier_loss_final.item(), 'loss_dmn': classifier_loss_domain.item(), 'loss_disc': disc_loss.item(), 'loss_remixed': classifier_remixed_loss.item(),}
...
class UltimateIRM(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.memory_bank = TemporalMemoryBank(
            ...
            momentum=self.hparams.get("ultimateirm_memory_momentum", 0.9),
        )
        ...
        self.guided_tmp = GuidedTMPMasker(
        ...
        self._latest_refresh_stats = {}
        self._latest_sample_rows = []
    ...
    def _irm_penalty_single_env(self, logits, y, w):
        ...
        l1 = (l1 * w1).sum() / (w1.sum() + 1e-8)
        l2 = (l2 * w2).sum() / (w2.sum() + 1e-8)

        g1 = autograd.grad(l1, [scale], create_graph=True)[0]
        g2 = autograd.grad(l2, [scale], create_graph=True)[0]
        return torch.sum(g1 * g2)

    def needs_env_refresh(self, step):
        warmup_steps = self.hparams.get("ultimateirm_warmup_steps", 1000)
        refresh_freq = self.hparams.get("ultimateirm_env_refresh_freq", 1000)

        if step < warmup_steps:
            return False
        if self.last_env_refresh_step < 0:
            return True
        return (step - self.last_env_refresh_step) >= refresh_freq
    ...
    def refresh_unlabeled_environments(self, uda_full_loaders, device, step):
        ...
        for loader in uda_full_loaders:
            for batch in loader:
                ...
                feats = self.featurizer(x_weak)
                logits = self.classifier(feats)
                probs = F.softmax(logits, dim=1)

                all_feats.append(feats.detach().cpu())
                all_probs.append(probs.detach().cpu())
                all_uids.extend(list(uid))
                all_y_true.append(y_true.detach().cpu())
        ...
        y_true_t = torch.cat(all_y_true, dim=0)
        ...
        pseudo, conf = compute_confidence(
            ...
            gamma=self.hparams.get("ultimateirm_temporal_gamma", 5.0),
            beta=self.hparams.get("ultimateirm_resdisp_beta", 5.0),
            dispersion_thresh=self.hparams.get("ultimateirm_resdisp_thresh", 0.05),
            w_max=self.hparams.get("ultimateirm_conf_w_max", 1.0),
            w_tmp=self.hparams.get("ultimateirm_conf_w_tmp", 1.0),
            w_res=self.hparams.get("ultimateirm_conf_w_res", 1.0),
        )

        cluster_ids, cluster_meta = run_clustering(
            mode=self.hparams.get("ultimateirm_cluster_mode", "A"),
            features=feats_np,
            k=self.hparams.get("ultimateirm_k", 3),
            seed=0,
        )

        self.env_assignments = {uid: int(cid) for uid, cid in zip(all_uids, cluster_ids)}
        ...
        cluster_sizes = dict(Counter(int(c) for c in cluster_ids))
        self._latest_refresh_stats = {
            "step": int(step),
            "stage": "causal_optimization",
            "num_samples": int(len(all_uids)),
            "conf_mean": float(conf.mean().item()),
            "conf_std": float(conf.std().item()),
            "cluster_sizes": cluster_sizes,
            "num_ood_gt": int((y_true_t < 0).sum().item()),
        }

        self.last_env_refresh_step = step
        self.stage = "causal_optimization"
        self.train()

    def update(self, minibatches, unlabeled=None, step=None):
        self.update_count += 1

        all_x = torch.cat([b["x"] for b in minibatches], dim=0)
        all_y = torch.cat([b["y"] for b in minibatches], dim=0)

        logits_all = self.predict(all_x)
        sup_loss = F.cross_entropy(logits_all, all_y)

        labeled_weights, _ = compute_env_weights(
            num_labeled_envs=len(minibatches),
            unlabeled_cluster_sizes={},
            lambda_labeled=self.hparams.get("ultimateirm_lambda_labeled", 1.0),
            lambda_unlabeled=0.0,
            size_norm=self.hparams.get("ultimateirm_size_norm", True),
        )

        pen_a = torch.tensor(0.0, device=all_x.device)
        for env_idx, b in enumerate(minibatches):
            logits_env = self.predict(b["x"])
            w_env = torch.ones_like(b["y"], dtype=torch.float32, device=all_x.device)
            pen_a = pen_a + labeled_weights[env_idx] * self._irm_penalty_single_env(
                logits_env, b["y"], w_env
            )
        ...
        if unlabeled is not None and len(unlabeled) > 0:
            xw = torch.cat([u["x_weak"] for u in unlabeled], dim=0)
            xs = torch.cat([u["x_strong"] for u in unlabeled], dim=0)
            xm = torch.cat([u["x_mask"] for u in unlabeled], dim=0)
            y_true = torch.cat([u["y_true"] for u in unlabeled], dim=0)
            uid = sum([list(u["uid"]) for u in unlabeled], [])

            with torch.no_grad():
                probs_w = F.softmax(self.predict(xw), dim=1)
                pseudo, conf = compute_confidence(
                    mode=self.hparams.get("ultimateirm_confidence_mode", "A"),
                    probs=probs_w,
                    uid_list=uid,
            ...
            if self.stage == "causal_optimization" and len(self.env_assignments) > 0:
                env_ids = torch.tensor(
                    [self.env_assignments.get(u, -1) for u in uid],
                    device=all_x.device,
                    dtype=torch.long,
                )

                min_cluster_size = self.hparams.get("ultimateirm_min_cluster_size", 4)
                for k in range(self.hparams.get("ultimateirm_k", 3)):
                    count_k = int((env_ids == k).sum().item())
                    if count_k >= min_cluster_size:
                        cluster_sizes[k] = count_k

                _, unlabeled_weights = compute_env_weights(
                    num_labeled_envs=len(minibatches),
                    unlabeled_cluster_sizes=cluster_sizes,
                    lambda_labeled=0.0,
                    lambda_unlabeled=self.hparams.get("ultimateirm_lambda_unlabeled", 1.0),
                    size_norm=self.hparams.get("ultimateirm_size_norm", True),
                )

                for k, w_k in unlabeled_weights.items():
                    mk = (env_ids == k)
                    pen_b = pen_b + float(w_k) * self._irm_penalty_single_env(
                        logits_s[mk], pseudo_dev[mk], conf_dev[mk]
                    )
            ...
            for i, u in enumerate(uid):
                latest_rows.append({
                    ...
                    "pseudo": int(pseudo_dev[i].item()),
                    "confidence": float(conf_dev[i].item()),
                    "accepted": int(accept_mask[i].item() > 0),
                    "tmp_applied": int(tmp_applied_mask[i].item()),
                    "cluster_id": int(self.env_assignments.get(u, -1)),
                    "is_ood_gt": int(y_true[i].item() < 0),
                })

        self._latest_sample_rows = latest_rows

        total_loss = (
            sup_loss
            + pen_a
            + self.hparams.get("ultimateirm_lambda_u_strong", 1.0) * loss_u_strong
            + self.hparams.get("ultimateirm_lambda_u_mask", 1.0) * loss_u_mask
            + pen_b
        )
        ...
        out = {
            ...
            "loss_u_strong": float(loss_u_strong.item()),
            "loss_u_mask": float(loss_u_mask.item()),
            "penalty_a": float(pen_a.item()),
            "penalty_b": float(pen_b.item()),
            "stage_id": 0 if self.stage == "warmup" else 2,
        }

        if len(latest_rows) > 0:
            conf_tensor = torch.tensor([r["confidence"] for r in latest_rows], device=all_x.device)
            out["conf_mean"] = float(conf_tensor.mean().item())
            out["conf_std"] = float(conf_tensor.std().item())
            out["pseudo_accept_rate"] = float(
                np.mean([r["accepted"] for r in latest_rows])
            )
            out["tmp_applied_rate"] = float(
                np.mean([r["tmp_applied"] for r in latest_rows])
            )
    ...
    def export_diagnostics(self, output_dir, step):
        diag_dir = os.path.join(output_dir, "diagnostics", f"step_{step:07d}")
        os.makedirs(diag_dir, exist_ok=True)

        self.memory_bank.export_csv(os.path.join(diag_dir, "memory_bank.csv"))

        with open(os.path.join(diag_dir, "env_assignments.json"), "w", encoding="utf-8") as f:
            json.dump(self.env_assignments, f, indent=2)

        with open(os.path.join(diag_dir, "refresh_stats.json"), "w", encoding="utf-8") as f:
            json.dump(self._latest_refresh_stats, f, indent=2)

        if len(self._latest_sample_rows) > 0:
            csv_path = os.path.join(diag_dir, "unlabeled_samples.csv")
