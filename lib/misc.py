        ...
        for g in self.param_groups:
            for p in g['params']:
                ...
                if p.ndim > 1:
                    ...
                    q = torch.where(param_norm > 0., torch.where(update_norm > 0, (g['trust_coefficient'] * param_norm / update_norm), one), one)
                    dp = dp.mul(q)
...
class SupConLossLambda(torch.nn.Module):
    ...
    def forward(self, features: torch.Tensor, labels: torch.Tensor, domain_labels: torch.Tensor) -> torch.Tensor:
        ...
        for i, (label, domain_label) in enumerate(zip(labels, domain_labels)):
            cond_pos_in_domain = torch.logical_and(labels==label, domain_labels == domain_label)
            cond_pos_in_domain[i] = False
            ...
            num_positives = pos_feats_in_domain.shape[0] + pos_feats_out_domain.shape[0]
            log_fraction = torch.log((scaled_exp_term / scaled_denom_const) + 1e-5)
            loss_i = torch.sum(log_fraction) / num_positives
            if torch.isnan(loss_i):
                nans += 1
                continue
            loss -= loss_i
        return loss/(batch_size-nans+1)
