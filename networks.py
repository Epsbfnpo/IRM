...
class MLP(nn.Module):
    def __init__(self, n_inputs, n_outputs, hparams):
        ...
        self.hiddens = nn.ModuleList([nn.Linear(hparams['mlp_width'], hparams['mlp_width']) for _ in range(hparams['mlp_depth']-2)])
...
class DinoV2(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        linear_input = torch.cat([x[0][1], x[1][1], x[2][1], x[3][1], x[3][0].mean(1)], dim=1)
...
class ContextNet(nn.Module):
    def __init__(self, input_shape):
        ...
        padding = (5 - 1) // 2
        self.context_net = nn.Sequential(nn.Conv2d(input_shape[0], 64, 5, padding=padding), nn.BatchNorm2d(64), nn.ReLU(), nn.Conv2d(64, 64, 5, padding=padding), nn.BatchNorm2d(64), nn.ReLU(), nn.Conv2d(64, 1, 5, padding=padding),)
...
def Classifier(in_features, out_features, is_nonlinear=False):
    if is_nonlinear:
        return torch.nn.Sequential(torch.nn.Linear(in_features, in_features // 2), torch.nn.ReLU(), torch.nn.Linear(in_features // 2, in_features // 4), torch.nn.ReLU(), torch.nn.Linear(in_features // 4, out_features))
    else:
