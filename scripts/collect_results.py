...
def merge_records(records):
    ...
    for record in records:
        ...
        args.pop('holdout_fraction', None)
...
def print_results_tables(records, selection_method, latex):
    ...
    grouped_records = grouped_records.map(lambda group:
        { **group, "sweep_acc": selection_method.sweep_acc(group["records"]) }).filter(lambda g: g["sweep_acc"] is not None)
...
if __name__ == "__main__":
    ...
    if args.auto_lr:
        ...
        SELECTION_METHODS = [model_selection.IIDAccuracySelectionMethod, model_selection.LeaveOneOutSelectionMethod, model_selection.OracleSelectionMethod,]
