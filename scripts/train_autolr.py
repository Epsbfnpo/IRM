...
if __name__ == "__main__":
    ...
    train_loaders = [InfiniteDataLoaderWithoutReplacement(dataset=env, weights=env_weights, batch_size=hparams['batch_size'], num_workers=dataset.N_WORKERS) for i, (env, env_weights) in enumerate(in_splits) if i not in args.test_envs]
    uda_loaders = [InfiniteDataLoaderWithoutReplacement(dataset=env, weights=env_weights, batch_size=hparams['batch_size'], num_workers=dataset.N_WORKERS) for i, (env, env_weights) in enumerate(uda_splits)]
    eval_loaders = [FastDataLoader(dataset=env, batch_size=64, num_workers=dataset.N_WORKERS) for env, _ in (in_splits + out_splits + uda_splits)]
    eval_loaders_iid = [FastDataLoader(dataset=env, batch_size=64, num_workers=dataset.N_WORKERS) for i, (env, _) in enumerate(out_splits) if i not in args.test_envs ]
    ...
    for step in range(start_step, n_steps):
        ...
        if (step % checkpoint_freq == 0) or (step == n_steps - 1):
           ...
           if schedule is not None and schedule[-1] == [0.0]:
               break
           results = {'step': step, 'epoch': step / steps_per_epoch, 'lr': algorithm.optimizer.param_groups[0]['lr']}
    ...
    train_loaders = [InfiniteDataLoaderWithoutReplacement(dataset=env, weights=env_weights, batch_size=hparams['batch_size'], num_workers=dataset.N_WORKERS) for i, (env, env_weights) in enumerate(in_splits) if i not in args.test_envs]
    uda_loaders = [InfiniteDataLoaderWithoutReplacement(dataset=env, weights=env_weights, batch_size=hparams['batch_size'], num_workers=dataset.N_WORKERS) for i, (env, env_weights) in enumerate(uda_splits)]
    eval_loaders = [FastDataLoader(dataset=env, batch_size=64, num_workers=dataset.N_WORKERS) for env, _ in (in_splits  + uda_splits)]
    eval_weights = [None for _, weights in (in_splits  + uda_splits)]
    eval_loader_names = ['env{}_in'.format(i) for i in range(len(eval_loaders))]
    eval_loader_names += ['env{}_uda'.format(i) for i in range(len(uda_splits))]
    algorithm_class = algorithms.get_algorithm_class(args.algorithm)
    algorithm = algorithm_class(dataset.input_shape, dataset.num_classes, len(dataset) - len(args.test_envs), hparams)
    if algorithm_dict is not None:
    ...
    def save_checkpoint(filename):
        if args.skip_model_save:
            return
        save_dict = {"args": vars(args), "model_input_shape": dataset.input_shape, "model_num_classes": dataset.num_classes, "model_num_domains": len(dataset) - len(args.test_envs), "model_hparams": hparams, "model_dict": algorithm.state_dict()}
    ...
    for step in range(start_step, n_steps):
        ...
        for key, val in step_vals.items():
            checkpoint_vals[key].append(val)
        if (step % checkpoint_freq == 0) or (step == n_steps - 1):
            results = {'step': step, 'epoch': step / steps_per_epoch, 'lr': float(algorithm.optimizer.param_groups[0]['lr'])}
